...
public class ExcelUtil<T> {
	...
	public List<T> importExcel(String fileName, InputStream input,int readLine) throws Exception  {
		...
		for (int s = 0; s < workbook.getNumberOfSheets(); s++) {
			...
			if (rows > 0) {// 有数据时才处理
				...
				for (int i = readLine-1; i < rows; i++) {// 从第2行开始取数据,默认第一行是表头.
					Row row = sheet.getRow(i);
					int cellNum = row.getLastCellNum(); 
					//getLastCellNum 获取的是最后一列不为空的所有列   getPhysicalNumberOfCells获取不为空的所有列
					T entity = null;
						for (int j = 0; j < cellNum; j++) {
							...
							String c=getStringValueCell(cell);
							//String c = cell.getStringCellValue();
							// System.out.println(c);
							if (c.equals("")) {
								continue;
							}
							entity = (entity == null ? clazz.newInstance()
									: entity);
							// 如果不存在实例则新建.
							...
							{
							    field.set(entity,MathUtils.numObj2BigDecimal(c));
	...
	public String getStringValueCell(Cell cell){
		String cellValue = "";
		...
		if (null != cell) {
			...
			case Cell.CELL_TYPE_FORMULA: // 公式
				try {
					 ...
					 * (cell)判断过程中cell
					 * .getNumericCellValue();方法会抛出java.lang
					 * .NumberFormatException异常
					 */
					if (HSSFDateUtil.isCellDateFormatted(cell)) {
						Date date = cell.getDateCellValue();
						calendar.setTime(date);
						cellValue = (calendar.get(Calendar.DAY_OF_YEAR)  + 1900) + "-"
